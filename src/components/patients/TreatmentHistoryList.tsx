...
export function TreatmentHistoryList({
  ...
  return (
    <div className="space-y-3">
      ...
      {sorted.length === 0 ? (
        <p className="text-sm text-foreground/60">Sin tratamientos registrados.</p>
      ) : (
        <ul className="space-y-2">
          {sorted.map((event) => (
            <li key={event.id} className="rounded border border-border p-2 text-sm">
              ...
              {event.notes && <p className="text-foreground/60">{event.notes}</p>}
            </li>
          ))}
        </ul>
      )}
